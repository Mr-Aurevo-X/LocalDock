# Conditions générales d’utilisation — LocalDock

Éditeur : Mr-Aurevo-X  
Produit : LocalDock  
Copyright (c) 2026 Mr-Aurevo-X. Tous droits réservés.

Les présentes CGU s’appliquent à **LocalDock**. Le fichier anglais `LICENSE` (PolyForm Noncommercial 1.0.0) est la licence. Ce document ajoute les règles d’usage. Ce n’est pas une seconde licence.
...
Binaires officiels : GitHub Releases `Mr-Aurevo-X/LocalDock`.
...
Vous assumez **100 %** du risque lié aux processus que vous lancez, à leur bind réseau et à leur configuration.

## 4. Responsabilité de l’utilisateur

LocalDock oriente les serveurs enfants vers `127.0.0.1` quand `force_loopback` est actif, mais un projet peut encore écouter hors loopback. Vous confirmez les actions destructives (ex. tuer un PID). Windows peut afficher un avertissement SmartScreen : les binaires ne sont pas signés Authenticode.

## 5. Soutien facultatif

Liens Discord, crypto (in-app) : **volontaires**. Un don n’est pas un prix de licence et n’achète ni support ni mises à jour. Un clic quitte l’application locale.
